...
DeviceIteratorMacOSX::DeviceIteratorMacOSX() {
	...
	result = IOServiceGetMatchingServices( kIOMasterPortDefault, matchingDictionary,
	    &deviceIterator );
...
FirewireDevice* DeviceIteratorMacOSX::next(){
	...
	for(;;) {
		...
		if (!unitIterator) { //If there are no more units in this device
			//Get the next device
			device = IOIteratorNext( deviceIterator );
			if (!device) return NULL; //There are no more devices, return NULL

			//Get the iterator to iterate over this device
			result = IORegistryEntryGetChildIterator (device, kIOServicePlane,
					&unitIterator);
        ...
        if (strcmp (name, "IOFireWireUnit")) { //If it is *not* an IOFireWireUnit
        ...
        SInt32 score;
        result = IOCreatePlugInInterfaceForService (unit, kIOFireWireLibTypeID,
...
FirewireDeviceMacOSX::FirewireDeviceMacOSX(IOFireWireLibDeviceRef interface): interface(interface) {
